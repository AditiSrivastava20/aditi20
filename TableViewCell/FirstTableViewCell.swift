...
class FirstTableViewCell: UITableViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    override func awakeFromNib() {
        ...
        firstCollectionView.register(nib, forCellWithReuseIdentifier: "CollectionCell2")
        // Initialization code
    ...
    {
        let Collectioncell2 = collectionView.dequeueReusableCell(withReuseIdentifier: "CollectionCell2", for : indexPath) as! secondCollectionViewCell
        let url = URL(string: datafill?[indexPath.row].image ?? "")
        print(url)
       
        Collectioncell2.secondCVimage.kf.setImage(with: url)
        Collectioncell2.secondCVlabel.text = datafill?[indexPath.row].name
        Collectioncell2.layer.cornerRadius = 5
        print(datafill?[indexPath.row].name)
